...
(function (global) {
  ...
  const DICT = {
    zh: {
      ...
      uploadErr: '未能从该文件夹读取 metadata.json，请确认包含了 demo/frames 数据',
      fieldPressure: '压力',
      fieldDensity: '密度',
      fieldSpeed: '速度',
    },
    en: {
      ...
      uploadErr: 'Could not read metadata.json from this folder. Make sure it contains the demo/frames data',
  ...
  const I18N = {
    ...
    apply() {
      ...
      opts.forEach((el) => { el.textContent = this.t(el.getAttribute('data-i18n')); });
